/**
 * Serviço de análise com IA (regras de negócio).
 *
 * Este módulo monta prompts/contextos e interpreta resultados; TODO o
 * acesso ao SDK acontece via services/llm.js (Fase 1 da SPEC-ANALISE-IA.md):
 * - saída estruturada com responseSchema (sem parse manual de texto livre)
 * - modelo resolvido por tarefa (escolha do usuário sempre vence)
 * - o sistema multi-agentes foi aposentado (itens A1/A2/D4 da spec)
 */

const llm = require("./llm");
const { VIDEO_ANALYSIS_SCHEMA } = require("../schemas/videoAnalysis");
 ...
 * @returns {string} Texto formatado com as regras
 */
function formatBeltRules(belt) {
  const rules = resolveBeltRules(belt);

  if (!rules) {
    // Faixa não informada ou desconhecida: nunca deixar o consumidor sem
    // NENHUMA orientação — aplicar o conjunto mais restritivo (branca)
    // como fallback seguro, já que assumir uma faixa mais permissiva
    // arrisca sugerir/validar técnica ilegal.
...
/**
 * Normaliza URL curta youtu.be → youtube.com/watch?v= (formato que a doc
 * do Gemini usa para fileData de YouTube).
 */
...
/**
 * Pós-processa a análise estruturada: garante que cada gráfico some 100%
 * e descarta gráficos sem nenhum dado observado.
 */
function normalizeAnalysisCharts(analysis) {
  if (Array.isArray(analysis.charts)) {
    analysis.charts = analysis.charts
      .map(chart => ({ ...chart, data: normalizeChartData(chart.data) }))
...
/**
 * Analisa um vídeo de luta com o Gemini (uma chamada, saída estruturada).
 *
 * @param {string} url - URL do YouTube ou Data URI (imagem/vídeo em base64)
 * @param {Object} context - Contexto adicional (athleteName, giColor, belt, matchResult)
 * @param {string|null} customModel - Modelo escolhido pelo usuário (opcional)
 * @returns {Promise<{analysis: Object, usage: Object}>}
 */
async function analyzeFrame(url, context = {}, customModel = null) {
  const model = resolveModel('VIDEO_ANALYSIS', customModel);

  const isYouTube = url.includes('youtube.com/') || url.includes('youtu.be/');
  const basePrompt = getPrompt('video-analysis', { VIDEO_URL: isYouTube ? url : '[MÍDIA ANEXADA]' });
  const textPrompt = basePrompt + buildVideoAnalysisContext(context);

  // Cleanup de artefatos temporários (download local + File API)
  ...
  const callWithMediaPart = async (mediaPart) => {
    const { data, usage } = await llm.generateJson({
      ...
      temperature: GENERATION.JSON_TEMPERATURE,
    });
    return { analysis: normalizeAnalysisCharts(data), usage };
  };

  try {
    // Data URI (imagem/vídeo embutido em base64)
    if (url.startsWith('data:')) {
      const match = url.match(/^data:(.+?);base64,(.+)$/);
      if (!match) {
        throw new Error('Data URI inválido - formato não reconhecido');
      }
      console.log('📷 [analyzeFrame] Mídia anexada ao prompt (inlineData)');
      return await callWithMediaPart({ inlineData: { mimeType: match[1], data: match[2] } });
    }

    // YouTube: tenta a URL direto (vídeos públicos); fallback para
    // download local + upload via File API se o Gemini recusar.
    if (isYouTube) {
      const youtubeUrl = normalizeYouTubeUrl(url);
      try {
        console.log('🎬 [analyzeFrame] Tentativa 1: URL do YouTube direto para o Gemini...');
        return await callWithMediaPart({ fileData: { fileUri: youtubeUrl } });
      } catch (directError) {
        console.warn(`⚠️ [analyzeFrame] URL direto falhou (${directError.message}). Tentativa 2: download + File API...`);

        const downloaded = await downloadYouTubeVideo(url);
        videoCleanup = downloaded.cleanup;

        const uploaded = await llm.uploadVideo(downloaded.filePath);
        geminiFileName = uploaded.name;

        console.log('🎬 [analyzeFrame] Vídeo enviado via File API:', uploaded.uri);
        return await callWithMediaPart({ fileData: { fileUri: uploaded.uri, mimeType: uploaded.mimeType } });
      }
    }
    ...
    console.error('❌ Erro ao analisar vídeo:', error.message);
    throw parseGeminiError(error);
  } finally {
    if (videoCleanup) videoCleanup();
    if (geminiFileName) llm.deleteFile(geminiFileName);
  }
}
...
/**
 * Consolida múltiplas análises de vídeos em uma única análise agregada
 * (função pura, sem IA)
 * @param {Array<Object>} frameAnalyses - Array de análises
 ...
 * @param {string|null} customModel - Modelo escolhido pelo usuário
...
async function consolidateSummariesWithAI(summaries, athleteName, customModel = null) {
  ...
  try {
    ...
    return summaries.join(' '); // Fallback: concatenar
  }
}
 ...
 * @param {Object} athleteData - Dados do atleta (name, belt, resumo, technical_stats)
 * @param {Object} opponentData - Dados do adversário (name, belt, resumo, technical_stats)
 * @param {string|null} customModel - Modelo escolhido pelo usuário
 * @returns {Promise<{strategy: Object, usage: Object}>}
 */
async function generateTacticalStrategy(athleteData, opponentData, customModel = null) {
  const model = resolveModel('STRATEGY', customModel);
...
Se sugerir leg lock, verifique se é permitido para a faixa.`;
  ...
  try {
    const { data: strategy, usage } = await llm.generateJson({
      ...
      temperature: GENERATION.STRATEGY_TEMPERATURE,
    });
...
function buildChatSystemPrompt(contextType, contextData) {
  if (contextType === 'analysis') {
    return getPrompt('chat-analysis', {
      ...
      TECHNICAL_STATS: JSON.stringify(contextData.technical_stats || contextData.technicalStats || {}, null, 2),
 ...
 * @param {Array} params.history - Histórico [{role: 'user'|'model', content: string}]
 * @param {string} params.userMessage - Nova mensagem do usuário
 * @param {string|null} params.customModel - Modelo escolhido pelo usuário
