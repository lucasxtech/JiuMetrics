...
| Severidade | Problema |
|---|---|
| **HIGH** | **Sink de XSS no export de PDF.** `pages/Analyses.jsx` monta HTML interpolando conteúdo de estratégia gerado por IA e faz `tempDiv.innerHTML = content` + `appendChild(document.body)`. `innerHTML` não executa `<script>`, mas **executa handlers** (`<img onerror>`). Com o JWT em `localStorage`, XSS = roubo de sessão válida por 7–30 dias. O conteúdo vem de LLM sobre **vídeo de terceiros** |
