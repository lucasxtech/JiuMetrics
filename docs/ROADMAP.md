...
**Projeto no Claude Design:** [`Design scope questionnaire`](https://claude.ai/design/p/6f39984b-2862-4cac-b7e2-8d93de97ffc6?file=JiuMetrics.dc.html) (dono Lucas Menezes). Arquivos: `JiuMetrics.dc.html` (protótipo clicável completo), `StrategyReport.dc.html` (relatório de estratégia como componente único para tela e PDF), **`Login.dc.html`** (2026-09-23: login final com a **logo StartDoing**, erros de senha e de conta desativada, sem criar conta), **`Perfis e acessos.dc.html`** (2026-09-23: matriz de acesso por perfil, navegação e Início por perfil, tela de Usuários completa, proposta da aba Nutrição), `jm-icons.js`, `support.js`, `github.md` (mapa protótipo → arquivos do repo).
...
- **Marca:** a logo passa a ser a da **StartDoing** (`assets/startdoing-logo.jpeg` no projeto do Claude Design; no painel escuro entra com `filter:invert(1)`), no lugar do quadrado "JJ". O nome do produto continua JiuMetrics. Rodapé do login: "Acesso restrito às contas da equipe StartDoing" e versão em mono (`v2.0`).
- **Login:** duas colunas; à esquerda logo grande, título, três linhas de recursos; à direita e-mail, senha com mostrar/esconder, switch "Manter-me conectado", botão com spinner e rótulo "Entrando". **Não há "criar conta" nem "esqueci a senha"**: o texto diz para falar com o admin. Erros inline com `role=alert`: campo vazio, "E-mail ou senha incorretos", "Esta conta está desativada".
...
| R-13 | ✅ **Implementado.** Exclusão apaga tudo — fichas geridas/vinculadas, análises, versões, estratégias, chats e adversários, dentro do tenant. Sem transferência (`transferToUserId` no corpo é 400). `api_usage` preservado. **Decisão do controller, não da spec original, que o dono deve confirmar:** ficha gerida pela conta excluída mas vinculada a outra conta viva do tenant é **reparentada**, não apagada; e análises que a conta fez de pessoas que continuam são **transferidas** ao gestor delas (revisão final) — ver §4 | decisão | R-04 | ✅ |
...
| R-21 | Design system em código: tokens (§2.1), **logo StartDoing exportada do projeto do Claude Design para `frontend/public/brand/`** (JPEG; pedir versão SVG ou PNG com fundo transparente), Geist, componentes base, badge de faixa (portar cores de `constants/persons.js`), badge de estimativa de IA, sheet acessível, toast com desfazer, skeleton, vazio, erro | código | R-20 | ⚪ |
...
| R-23 | Login conforme `Login.dc.html` (erros de credencial e de conta desativada vindos do backend, sem enumerar e-mail), **404**, **sessão expirada**. Cadastro público sai da interface; `ALLOW_PUBLIC_REGISTER` continua desligado | código | R-22 | ⚪ |
...
| — | **Transferir (em vez de apagar) as análises que a conta excluída fez de pessoas que continuam?** Decidido pelo controller na revisão final da spec 014 (2026-09-24), divergindo da spec: `fight_analyses`/`profile_versions` escritas pela conta sobre uma ficha ou adversário que sobrevive à purga (ex.: o atleta autogerido que o professor analisou) passam para o gestor dessa pessoa (`deleted.reassignedAnalyses`/`reassignedProfileVersions`), espelhando o reparent. Pessoa apagada junto, ou não encontrada no tenant, continua apagando a linha | se o dono discordar, a transferência sai de `User.js#purgeAccount` e essas linhas voltam a ser apagadas — o atleta perde o histórico de análise que o professor excluído tinha feito dele |
