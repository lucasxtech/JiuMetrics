...
| Funcionalidade | Notas |
|---|---|
| CRUD de atletas e adversários | Entidades estruturalmente idênticas, com **uma** implementação desde a spec 012 (`personModel`/`personController`); `name` e `belt` obrigatórios, validados por zod |
...
Chat é o único caminho de IA sem `responseSchema` (parsing por regex que **escreve no banco**) · ~~`handleError` vaza `error.message`~~ ✅ **resolvido na spec 007** (omitido em produção; o detalhe fica só no log do servidor) · enumeração de usuários no login · PII em log · ~~sem `helmet`/CSP~~ ✅ **resolvido na spec 010** (`helmet` deliberado; CSP no frontend em Report-Only) · CORS aceita qualquer `*.vercel.app` · validação de host do YouTube por substring no **backend** (frontend corrigido na spec 010; SSRF limitado continua no backend) · nenhum validador de schema de entrada nos ~8 endpoints fora de `/api/ai/*` e de atletas/adversários (estes validados na spec 012) · sem `UNIQUE` em nenhuma migration · ~~dois clientes Supabase com divisão arbitrária e fallback silencioso~~ ✅ **resolvido na spec 008** (cliente único `service_role`; o `REVOKE` de acesso segue pendente — ver HIGH acima) · prompt de produção hardcoded fora de `services/prompts/` · dois padrões de fetch no frontend sem invalidação cruzada (2 das 5 páginas migradas: `Overview` na spec 010, `AthleteDetail` na 012) · dashboard carrega 3 tabelas para exibir 4 números · sem paginação nas listagens · `athletes` e `opponents` duplicados no **banco** (o código é um só desde a spec 012) · migrations com PII e `UPDATE` destrutivo sem `WHERE` · dois destinos de deploy simultâneos.
...
| **Dívida de lint documentada em código** | A spec 003 tornou o lint bloqueante sem corrigir o que exige decisão de comportamento. Há `eslint-disable` **com comentário apontando a spec responsável** em: `versionManager` (3× — evidência dos bugs das specs 006/007), `StrategyChatPanel` (callback nunca chamado — cluster F14/F19-F22), `VideoAnalysis` (`addVideo` sem controle na UI), `AuthContext` (`set-state-in-effect` na hidratação de sessão), `Strategy` (loading calculado e nunca renderizado). **Nenhum foi escondido com `_`** — a evidência fica visível |
| **Duplicação** | `processPersonAnalyses` (front × back, divergente) · `AVAILABLE_MODELS` (2 lugares) · ~~`Opponent.js` = cópia de `Athlete.js`~~ ✅ spec 012 · `chatLimiter` 2× |
