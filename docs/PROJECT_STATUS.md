...
| Funcionalidade | Notas |
|---|---|
| CRUD de atletas e adversários | Entidades estruturalmente idênticas, com **uma** implementação desde a spec 013 (`personModel`/`personController`); `name` e `belt` obrigatórios, validados por zod |
...
| **Duplicação** | `processPersonAnalyses` (front × back, divergente) · `AVAILABLE_MODELS` (2 lugares) · ~~`Opponent.js` = cópia de `Athlete.js`~~ ✅ spec 013 · `chatLimiter` 2× |
