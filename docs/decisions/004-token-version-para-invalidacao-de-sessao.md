...
- **⚠️ O fallback de erro anula as três proteções ao mesmo tempo.** Se `User.getAuthInfo` lançar (banco indisponível), o middleware continua com `req.user.role = decoded.role || 'user'` — isto é, **volta a confiar no token**. Nesse estado: token de conta desativada volta a valer, `token_version` deixa de ser comparado, e o `role` do token é aceito. É a falha **HIGH** AZ-8 da auditoria: um JWT antigo de admin só precisa que o banco fique instável. **Estreitado na revisão final da spec 014 (2026-09-24):** o fallback só vale para erro **sem** `code` (falha de rede/timeout); quando o banco responde — `PGRST116` (conta excluída) ou qualquer outro código —, o middleware devolve 401/503 e não olha o token. O caso de rede continua aberto.
